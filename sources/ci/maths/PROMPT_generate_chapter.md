...
- **`list_chapters`** — the list of chapters (numbering may skip; produce only chapters that exist).
- **`log_generation(...)`** — after the chapter `.docx` is finished, record what you produced (characters used, example domain used, concepts covered) so future chapters stay consistent and varied.

### How to build a chapter's content

1. Call `get_generation_context(N, "manual")` (or at minimum `get_curriculum(N)`).
