...
describe("installProcessGuards", () => {
  it("registers unhandledRejection + uncaughtException handlers that never rethrow", () => {
    ...
    expect(process.listeners("unhandledRejection").length).toBe(rejectionListenersBefore + 1);
    expect(process.listeners("uncaughtException").length).toBe(exceptionListenersBefore + 1);
  ...
  it("is idempotent — a second call adds no duplicate listeners", () => {
    const rejectionListenerCount = process.listeners("unhandledRejection").length;
    const exceptionListenerCount = process.listeners("uncaughtException").length;
    installProcessGuards("[test]");
    installProcessGuards("[test]");
    expect(process.listeners("unhandledRejection").length).toBe(rejectionListenerCount);
    expect(process.listeners("uncaughtException").length).toBe(exceptionListenerCount);
  });
});
