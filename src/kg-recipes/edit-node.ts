...
// Write the display title to the node's normalized field (title vs text by
// grouping-ness) + its raw mirrors. The one field that had no verb until now.
function applyTitle(graph: MutationGraph, args: EditNodeArgs): MutationGraph {
...
export const editNode: GraphMutation<EditNodeArgs> = {
  name: "editNode",
  describe: (args) => {
    const fields = (["content", "position", "title", "title_en"] as const).filter((field) => args[field] !== undefined);
  ...
  validate: (base, _after, args) => {
    ...
    const editsSomething = args.content !== undefined || args.position !== undefined || args.title !== undefined || args.title_en !== undefined;
    if (!editsSomething) {
      errors.push(`edit_node: provide at least one of content / position / title / title_en to edit.`);
    }
  ...
  apply: (base, args) => {
    ...
    if (args.title !== undefined || args.title_en !== undefined) {
      graph = applyTitle(graph, args);
    }
    return graph;
  },
};
