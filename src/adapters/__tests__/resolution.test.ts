...
describe("adapter resolution", () => {
  afterEach(() => {
    for (const [grade, subject] of TEST_KEYS) __registerAdapterForTest(grade, subject, null);
  });
  ...
  it("supports many-to-one: two (grade, subject) pairs can point at one adapter builder", () => {
    ...
    for (const [grade, subject] of TEST_KEYS) __registerAdapterForTest(grade, subject, buildCiMathsAdapter);
