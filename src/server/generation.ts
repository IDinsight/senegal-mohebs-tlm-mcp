 ...
 * here — generation reads it from the graph (walk_graph /
...
export function registerGenerationTools(server: McpServer) {
  server.registerTool("get_prompt", { title: "Get generation prompt", description: "Return the generation prompt for one of the active subject's deliverables (its DeliverableSpec.promptFile). 'deliverable' is a deliverable key — for CI maths, 'manual' or 'lessons'. The prompt tells you which graph tools to read (namespace_stats / walk_graph / get_standards) for the curriculum itself.", inputSchema: { deliverable: z.string() } },
