...
export function registerGraphTools(server: McpServer) {
  server.registerTool(
    "walk_graph",
    {
      ...
      description:
        "Traverse the active subject's graph from `fromId` by breadth-first search, following only the edge types you name and returning only the node labels you want — the single generic read for every 'list / find / enumerate / traverse' need. `direction`: 'out' follows edges from→to (a Course down to its parts), 'in' follows to→from (a standard up to its framework root), 'both' either. `edgeTypes` filters which edges to follow (empty ⇒ all); `nodeTypes` filters which nodes to RETURN (empty ⇒ all) — non-matching nodes are still traversed THROUGH, so filters compose. `maxDepth` (default 3, max 10) bounds the hops; `truncated:true` means the depth cap stopped the walk before exhaustion. `includeEdges` (default true) returns the traversed edges so you can rebuild the subgraph. Paginated: `limit` (default 100, max 500) + an opaque `cursor` (pass back `nextCursor` for the next page). `slot`: 'published' (default) reads the live graph; 'draft' reads UNPUBLISHED staged edits (curators/approvers only). Read-only. Examples: walk(fromId=<any standard>, direction='in', edgeTypes=['hasChild'], nodeTypes=['StandardsFramework']) → the framework root; walk(fromId=<courseId>, direction='out', edgeTypes=['hasPart','hasChild']) → the course subtree.",
      inputSchema: {
        fromId: z.string(),
