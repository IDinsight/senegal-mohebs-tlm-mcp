...
// CI maths; a week for CE1 reading). Values come from the active profile's
// curriculum, so the RETURNED shapes are still subject-specific (maths returns
// chapitreNum/leconNum, etc.) even though the tool names/params are neutral.
...
export function registerCurriculumTools(server: McpServer) {
  server.registerTool("list_units", { title: "List curriculum units", description: "All top-level curriculum units for the active subject (for maths: chapters — number, title, domain). Numbering may skip.", inputSchema: {} },
    guarded(async () => asJson(getActiveProfile().curriculum.listUnits())));

  server.registerTool("get_curriculum", { title: "Get unit curriculum", description: "The curriculum slice for one unit (for maths: a chapter's ordered lessons with components and tasks, the bilan lesson, and cross-unit progression). 'unit' is the unit's scope value (maths: the chapter number).", inputSchema: { unit: z.number().int() } },
    guarded(async (a: { unit: number }) => { const c = getActiveProfile().curriculum; const s = c.slice(a.unit); return s ? asJson({ ...(s as object), progression: c.progression(a.unit) }) : asJson({ error: `Unit ${a.unit} not found.` }); }));
