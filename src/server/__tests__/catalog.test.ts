...
const rNode = (id: string, label: string, raw: Record<string, unknown>): Omit<StoredNode, "slot"> =>
  ({ id, type: label, namespace: CATALOG_NAMESPACE, labels: [label], spine: false, properties: { raw } });
const rEdge = (from: string, to: string): Omit<StoredEdge, "slot"> =>
  ({ id: makeEdgeId("hasPart", from, to), type: "hasPart", from, to, namespace: CATALOG_NAMESPACE, properties: {} });

async function seedCatalog(s: KgNodeStore) {
  const nodes = [
    rNode("cat-root", "InstructionalRoutine", { description: "Routine library" }),
  ...
  const edges = [rEdge("cat-root", "cat-entry"), rEdge("cat-entry", "cat-s1"), rEdge("cat-entry", "cat-s2"), rEdge("cat-s1", "cat-m1"), rEdge("cat-s2", "cat-m2")];
  ...
  await s.writeSlot(CATALOG_NAMESPACE, "a", { nodes, edges, meta });
  await s.ensurePointer(CATALOG_NAMESPACE, "a");
}
...
describe("list_catalog", () => {
  it("reads the seeded catalog namespace and lists its entry", async () => {
    const entries = listCatalogEntries(await readCatalogGraph());
    expect(entries).toHaveLength(1);
    expect(entries[0]).toMatchObject({ id: "cat-entry", name: "Fiche de leçon", materialCount: 2 });
    expect(entries[0].steps.map((s) => s.id)).toEqual(["cat-s1", "cat-s2"]);
  });
});
