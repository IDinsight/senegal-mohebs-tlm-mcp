...
**Session read-model refresh (important).** The pointer flip is atomic *in the store*, but a live session does not read the store on every call: `activate.ts` hydrates the published `CurriculumModel` **once** at `set_context` and pins it in the session bag (`PRELOADED_MODEL_KEY`), and the adapter reads that snapshot synchronously. So a `publish_draft` in the same session must invalidate that snapshot, or reads keep serving the pre-publish slot even though the pointer moved. `publish_draft` does this: on a successful commit it calls `refreshActiveContext()`, which re-hydrates the published model (and rebuilds the adapter from the newly-published profile cell) from the now-current slot — the commit response reports `readModelRefreshed`. To make a read/write disagreement visible, `walk_graph` and `namespace_stats` echo `physicalSlot` — the actual slot (`a`/`b`) the returned data was hydrated from, stamped next to the model at hydration time (`PRELOADED_SLOT_KEY`). A cross-*session* publish is not auto-refreshed: another session already in-context keeps its own snapshot until it re-enters `set_context` (or publishes itself).
