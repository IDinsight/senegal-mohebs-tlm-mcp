...
subtree can equally be authored through the generic `add_node` + `set_content`
verbs rather than a splice script — the splice is just faster for a one-shot bulk add.

## Next phases (not in this PR)

1. **Feed the routine into generation.** Have the adapter's
   `buildGenerationContext` resolve `usesRoutine → steps → Material.content` and
   include the section specs in the payload, so a generation run reads the
   structure from the graph instead of from prompt prose.
