...
> **Status: Historical / partly superseded.** The central decision here — ship a **spine-only** store — was later reversed: the store now holds the FULL Learning-Commons graph (framework/derived nodes + `supports`/`relatesTo` edges included), and the explorer surfaces it. See [`../technical-reference/explorer.md`](../technical-reference/explorer.md) "Data-scope finding" and CLAUDE.md "Full-graph store" for the current design. Also SUPERSEDED: the subject-specific views (thematique/planification) and role-based legend described here — the explorer now follows the **LC ontology only** (nodes coloured by LC label; two generic views: containment hierarchy + by-label). See CLAUDE.md "Full-graph store". This note is kept for the original rationale and the feature-by-feature analysis.
