...
Curriculum edits are exposed as **composite recipes** (`add_lesson`/`add_lesson_grouping`/`move_lesson`/`split_lesson_grouping`/`renumber`, plus the Scope C content recipes `add_activity`/`add_material`/`set_material_content`) over the raw structural verbs — available only where the adapter declares a `recipeProfile`, and further narrowed by a per-subject `availableRecipes` allowlist (CI maths exposes the whole family; CE1 reading opts into `move_lesson` + the three content recipes, **not** week-level `split`/`renumber`). `add_lesson` creates a content lesson under a grouping **and aligns it (`supports`) to an existing `expectation`** (a lesson can't invent a standard); `add_lesson_grouping` creates an empty grouping with a `groupName` type. Grouping↔lesson membership is the `hasChild` **edge**, so move/split rewire the edge and renumber changes only the grouping's own number — no cross-lesson cascade. CI maths lessons carry a second axis: a `week → lesson` schedule edge alongside the `LessonGrouping → lesson` content edge, so a lesson legitimately has **two parents** (the multi-parent coverage rule is scoped to *grouping* parents; and force-deleting a grouping does not orphan its week-shared lessons). The **content layer inside a lesson** (Scope C, graph-native-authoring.md): `add_activity` puts an `Activity` (a phase/Étape) under a lesson via `hasPart` — **no** alignment edge, it inherits the lesson's standard; `add_material` puts a `Material` (the load-bearing `content` — prose/steps/image-brief) under an `Activity`, a `Lesson`, **or** a `LessonGrouping`; `set_material_content` rewrites an existing `Material`'s content. A `Material`'s `content` is **not** a `wordingAlias`, so `upsert_property` can't edit it — content is edited only through these two recipes (`MATERIAL_CONTENT_PATH`).
