...
- Adapters are **behavior only**: `detect`/`parse`, the LC→friendly projection (`listUnits`/`slice`/`progression`/`requiredCoverage`/`scopeValues`), `buildGenerationContext`, plus `deliverables`, `capabilities`, and the edit-surface declarations (`wordingAliases` for `upsert_property`, `coverageWarnings`). No schema/integrity lives on the adapter — write-safety is in the write tools. The curriculum recipes are **generic** now (`kg-recipes`), so an adapter no longer declares a `recipeProfile`/`structuralAliases`/`lcNodeTemplate`.
