...
- Adapters are **behavior only**: `detect`/`parse`, the LC→friendly projection (`listUnits`/`slice`/`progression`/`requiredCoverage`/`scopeValues`), `buildGenerationContext`, plus `deliverables`, `capabilities`, and the edit-surface declarations (`wordingAliases` for `upsert_property`, `coverageWarnings`). No schema/integrity lives on the adapter — write-safety is in the write tools. The curriculum recipes are **generic** now (`kg-recipes`), so an adapter no longer declares a `recipeProfile`/`structuralAliases`/`lcNodeTemplate`. Subject-agnostic adapter behavior — the model loader (`makeEnsure`), the generic `detect` (`detectEnvelope`), recurring-character aggregation, the lesson→aligned-standard index, and the wording-alias builder — lives in `src/adapters/engine.ts`; each subject module now supplies only what genuinely differs (its `GraphParseDescriptor` + read-projection shape + `deliverables`/config). The bilan flag is canonical (`educationalUse: "Assessment"` → `isAssessment`, set in `parseGraph`), not a per-subject hook.
- Each stored node keeps its raw LC fields under `properties.raw`, its top-level LC `labels`, and (spine nodes) normalized fields (`title`/`text`/`order`/`isAssessment`) alongside. `id` is the LC UUID verbatim. `upsert_property` edits the normalized field and its `raw.*` mirror atomically (`wordingAliases`). `add_node`-created nodes derive their LC identity (`labels`/`normalizedType`/`normalizedStatementType`/`metadata.role`, and the raw ordinal path(s)) by **copying an existing node of the same label from the graph** (canonical LC defaults when none exists yet), so they round-trip through the parser.
