  ...
  server.registerTool(
    "walk_graph",
    {
      ...
      description:
        ...
        "Each response carries `nextCursor` (null on the last page) plus two independent flags: `truncatedByLimit:true` means more matching nodes remain on further pages (call again with `cursor: <nextCursor>`), while `truncated:true` means the `maxDepth` cap hid deeper nodes (raise maxDepth to reach them). " +
