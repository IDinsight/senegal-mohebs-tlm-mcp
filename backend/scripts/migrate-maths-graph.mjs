...
const hasLabel = (n, lbl) => (n?.labels ?? []).includes(lbl);
...
const alreadySplit = nodes.some((n) => hasLabel(n, "Lesson"));
let chaptersConverted = 0, lessonsMinted = 0, repointed = 0, supportsAdded = 0;
...
if (!alreadySplit) {
  ...
  for (const [expId, lid] of lessonForExp) {
    ...
    rels.push({
      id, type: "supports", start: lid, end: expId,
      properties: {
...
// 5. Bilan as data (idempotent). Materialize the assessment flag onto each Lesson
//    node as LC `educational_use` ("Assessment" | "Instruction") — replacing the
//    former parse-time regex heuristic with an explicit graph property the parser
//    reads directly. Per grouping: the bilan is the highest-ordered lesson whose
//    aligned expectation's OS text mentions "bilan", else the highest-ordered
//    lesson. (One-time materialization of the old heuristic; from here it is data.)
function materializeBilan() {
  const byId = new Map(nodes.map((n) => [n.id, n]));
  const lessonsByGrouping = new Map();
  const expOfLesson = new Map();
  for (const r of rels) {
    if (r.type === "hasChild" && hasLabel(byId.get(r.start), "LessonGrouping") && hasLabel(byId.get(r.end), "Lesson"))
      (lessonsByGrouping.get(r.start) ?? lessonsByGrouping.set(r.start, []).get(r.start)).push(r.end);
    if (r.type === "supports" && hasLabel(byId.get(r.start), "Lesson")) expOfLesson.set(r.start, r.end);
  }
  const order = (id) => byId.get(id)?.properties?.metadata?.order ?? 0;
  const osText = (lid) => String(byId.get(expOfLesson.get(lid))?.properties?.description ?? "");
  // Default every lesson to Instruction; the per-grouping bilan flips to Assessment.
  for (const n of nodes) if (hasLabel(n, "Lesson")) n.properties.educational_use = "Instruction";
  let bilans = 0;
  for (const [, lessonIds] of lessonsByGrouping) {
    const sorted = [...lessonIds].sort((a, b) => order(a) - order(b));
    const bilan = [...sorted].reverse().find((l) => /bilan/i.test(osText(l))) ?? sorted[sorted.length - 1];
    if (bilan) { byId.get(bilan).properties.educational_use = "Assessment"; bilans++; }
  }
  return bilans;
}
const bilansStamped = materializeBilan();

console.log(JSON.stringify({
  alreadySplit, chaptersConverted, lessonsMinted, hasChildEdgesRepointed: repointed,
  supportsEdgesAdded: supportsAdded, bilansStamped, totalNodes: nodes.length, totalRels: rels.length,
}, null, 2));
